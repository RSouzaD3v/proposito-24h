...
export const HeaderReader = () => {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full flex items-center justify-center md:gap-10 gap-5 p-4 bg-[#202020] shadow rounded-b-[50px]">
      ...
      bg-gradient-to-l from-amber-400 to-amber-600 text-white font-bold rounded-full p-2">
        <h2>{user ? user?.name.charAt(0).toLocaleUpperCase() : "?"}</h2>
      </Link>

      <div>
        <h1 className="md:text-3xl text-xl text-white font-bold">Olá, {user?.name || "Usuário"}</h1>
        <p className="text-white">Vamos passar tempo com Deus?</p>
      </div>
    </header>
  );
};
