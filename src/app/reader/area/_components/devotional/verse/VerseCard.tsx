...
export const VerseCard = async () => {
  ...
  let userCompletionVerse: { id: string } | null = null;
  try {
    userCompletionVerse = await db.userCompletationVerse.findFirst({
      where: { userId: user.id, verseId: verse.id },
      select: { id: true },
    });
  } catch (e) {
    // Silencia erro de tabela ausente/coluna ausente para não derrubar SSR
    console.error("userCompletationVerse query failed:", e);
  }
