...
export default function AreaReader() {
  ...
  return (
    <section className="container mx-auto max-w-lg">
      <HeaderReader />
      <div className="space-y-6 md:p-0 p-6">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.link}
            className="flex items-center justify-between bg-gradient-to-r from-green-900 to-green-700 text-white p-5 rounded-2xl shadow-lg hover:scale-[1.03] hover:shadow-2xl transition-all duration-200 group"
          >
            <div>
