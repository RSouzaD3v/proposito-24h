  ...
  return (
    <section className="min-h-screen">
      ...
      <div className="mb-4 flex items-center gap-2">
        {nav.prev ? (
          <Link className="rounded-md border px-3 py-2 text-black bg-gray-100 hover:bg-gray-200" href={`/bible-acf/${b.abbrev}/${nav.prev}`}>
            ← Cap. {nav.prev}
          </Link>
        ) : <span className="rounded-md border text-black px-3 py-2 opacity-50">← Cap. —</span>}
        <Link className="rounded-md text-black border px-3 py-2 bg-gray-100 hover:bg-gray-200" href={`/bible-acf/${b.abbrev}`}>Capítulos</Link>
        {nav.next ? (
          <Link className="rounded-md border text-black px-3 py-2 bg-gray-100 hover:bg-gray-200" href={`/bible-acf/${b.abbrev}/${nav.next}`}>
            Cap. {nav.next} →
          </Link>
      ...
      <div className="mt-6 flex items-center gap-2">
        ...
        <Link className="rounded-md border px-3 py-2 text-black bg-gray-100 hover:bg-gray-200" href={`/bible-acf/${b.abbrev}`}>Capítulos</Link>
        {nav.next ? (
          <Link className="rounded-md border px-3 py-2 text-black bg-gray-100 hover:bg-gray-200" href={`/bible-acf/${b.abbrev}/${nav.next}`}>
            Cap. {nav.next} →
          </Link>
        ) : <span className="rounded-md border px-3 py-2 opacity-50">Cap. — →</span>}
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-6 flex items-center gap-2">
        <Link href={`/bible-nvi/${b.abbrev}/${cap}`} className="text-sm underline">Ver este capítulo em NVI</Link>
      </div>
    </section>
  );
}
