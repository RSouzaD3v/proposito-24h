...
import { quoteQueue } from "@/lib/queue";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    rows.forEach((r, i) => {
      try {
        const parsed = RowSchema.parse({
          nameAuthor: r.nameAuthor ?? r.autor ?? r.author,
          content: r.content ?? r.conteudo ?? r.citacao ?? r.quote,
          verse: r.verse ?? r.verso ?? r.referencia ?? r.reference,
          imageUrl: r.imageUrl ?? r.imagem ?? r.cover,
          createdAt: r.createdAt ?? r.data ?? r.date ?? new Date(), // fallback to current date
        });
    ...
    const jobs = await Promise.all(
      valid.map((data) => {
        const key = crypto.createHash("sha1").update(`${userWriter.writerId}|${data.nameAuthor}|${data.content}`).digest("hex");
        return quoteQueue.add("create-quote", data, { jobId: key });
      })
    );
